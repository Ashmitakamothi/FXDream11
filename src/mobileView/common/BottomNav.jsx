...
const BottomNav = () => {
  ...
  return (
    <nav className="fixed bottom-4 left-4 right-4 z-50">
      <div className="max-w-md mx-auto glass-effect rounded-2xl border border-border/30 px-1.5 py-1.5" style={{ boxShadow: '0 -2px 20px #0000000f, 0 4px 16px #00000014' }}>
        <div className="flex items-center justify-around">
          {navItems.map((tab) => {
            const active = isActivePath(tab.activeUrl);
            return (
              <button key={tab.label} onClick={() => navigate(tab.url)} className={`flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-xl transition-all duration-200 active:scale-90 ${active ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground/70"}`} style={active ? { boxShadow: '0 2px 10px #2bb67340' } : undefined }>
