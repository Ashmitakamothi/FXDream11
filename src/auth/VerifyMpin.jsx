...
import { verifyMpin } from "../api/authApi";
...
export default function VerifyMpin() {
  ...
  const handleVerifyMpin = async (values) => {
    ...
    try {
      ...
      const response = await verifyMpin(payload);
      ...
      console.error(err);
      message.error(err?.message || "Invalid MPIN. Please try again.");
    } finally {
    ...
    <Layout>
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, ease: [0.22, 1, 0.36, 1] }} className="hidden md:flex md:flex-1 md:min-w-0 md:max-w-lg lg:max-w-xl flex-col justify-center md:pr-6 lg:pr-8 xl:pr-10">
        <div className="relative w-full min-w-0 pl-1 sm:pl-2">
          ...
          <p className="text-muted text-base leading-relaxed max-w-md mb-10">{t("Enter your 6-digit MPIN to sign in.")}</p>
        ...
        <div className="w-full">
          ...
          <div className="login-card z-10 overflow-hidden rounded-2xl border shadow-2xl px-6 py-8 sm:px-8 sm:py-9" style={{ borderColor: "var(--theme-border)", background: "linear-gradient(165deg, var(--theme-card) 0%, var(--theme-light-bg) 100%)", boxShadow: "0 25px 50px -12px rgba(15, 23, 42, 0.12), 0 0 0 1px rgba(255,255,255,0.04) inset", }}>
            <div className="text-center mb-8">
              ...
              <h2 className="text-2xl font-bold text-theme tracking-tight">{t("Enter MPIN")}</h2>
              <p className="text-muted text-sm mt-2 leading-relaxed">{t("Enter your 6-digit MPIN")}</p>
            </div>
            ...
            <Form form={form} layout="vertical" className="login-input" onFinish={handleVerifyMpin}>
              <Form.Item label={<span className="text-theme label">{t("MPIN")}</span>} name="mpin" rules={[{ required: true, message: "Please enter your MPIN" }]}>
                <Input.OTP length={6} formatter={(str) => str.replace(/\D/g, "")} className="w-full" />
              </Form.Item>

              <LoadableButton htmlType="submit" lable={t("Verify & Sign In")} loadingLable={t("Verifying...")} isLoading={submitLoading} className="w-full login-btn text-white h-10 !rounded-lg" />
            </Form>
